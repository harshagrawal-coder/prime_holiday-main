import { CalendarDays, Minus, Plus, ClipboardList } from "lucide-react";
...
const Counter = ({ label, hint, value, min, onChange }) => (
  <div>
    ...
    <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-2 py-1.5">
      ...
      <span className="w-12 text-center text-xl font-black text-slate-900">{value}</span>
      <button
        type="button"
...
const TravelDetails = ({ value, onChange, errors = {} }) => {
  ...
  return (
    <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.06)] sm:p-8">
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-3">
        <div>
          ...
          {errors.departureDate && (
            <p className="mt-1.5 text-xs font-medium text-red-500">{errors.departureDate}</p>
          )}
        </div>
        ...
        <Counter
          label="Children"
          hint="Age 2 to 11, charged at 50%"
      ...
      <div className="mt-6">
        <label className="mb-2 block text-sm font-semibold text-slate-700">Special Request</label>
        <div className="relative">
          <ClipboardList size={15} className="pointer-events-none absolute left-4 top-4 text-slate-400" />
          <textarea
            name="specialRequest"
