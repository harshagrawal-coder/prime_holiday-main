...
import { getStartingPrice, getImageSrc, formatPrice } from "./tourUtils";
...
const TourCard = ({ tour, isHome = false }) => {
  ...
  const imageSrc = tour.thumbnail?.url || tour.image;
  ...
  const priceDisplay = price !== undefined
    ? getStartingPrice(price, discountPrice)
    : formatPrice(tour.priceRange?.match(/\d[\d,]*/)?.[0]);
